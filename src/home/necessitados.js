...
export function HomeNecessitados({ navigation, route }) {
  ...
  const mostrarDados = async () => {    const response = await axios.get(url + "/SocialHelp/Pessoas.php");
    setData(response.data.result);
  };
  mostrarDados();

  function cardsPessoas(value, index, array) {
  ...
  const mostrarDados2 = async () => {
    const response = await axios.get(url + "/SocialHelp/selectNecessitados2.php");
  ...
  return (
    <View style={styles.fundo}>
      <>
        <View style={{ flexDirection: "row", height: 0, right:80, left:5 }}>
        ...
        <ScrollView contentContainerStyle={styles.boxWhite}>
          <Text style={styles.text1}>Pessoas</Text>
          {data.map(cardsPessoas)}
